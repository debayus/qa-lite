...
import type { TestScenario, TestCase, TestCaseStatus } from "@/types";
...
export function useScenarios(projectId: string | undefined) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snap) => {
      const docs = snap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as TestScenario,
      );
  ...
  async function createScenario(title: string) {
  ...
  async function bulkDeleteScenarios(ids: string[]) {
  ...
  async function bulkCreateScenarios(titles: string[]) {
    if (!user || !projectId || titles.length === 0) return;
    const batch = writeBatch(db);
  ...
  async function bulkCreateScenariosWithTestCases(
    data: Array<{
      title: string;
      testCases: { title: string; steps: string[]; status: TestCaseStatus }[];
    }>,
  ) {
    if (!user || !projectId || data.length === 0) return;
    const batch = writeBatch(db);
    const startOrder = scenarios.length;
  ...
  async function mergeImportScenariosWithTestCases(
    data: Array<{
      title: string;
      testCases: { title: string; steps: string[]; status: TestCaseStatus }[];
    }>,
    ...
    const existingTcsSnap = await getDocs(
      query(collection(db, "test_cases"), where("projectId", "==", projectId)),
    );
    const tcCountByScenario = new Map<string, number>();
    existingTcsSnap.docs.forEach((d) => {
      const sid = d.data().scenarioId as string;
  ...
  async function fetchAllTestCases(): Promise<TestCase[]> {
    if (!projectId) return [];
    const q = query(
  ...
  async function moveScenario(id: string, direction: "up" | "down") {
    const idx = scenarios.findIndex((s) => s.id === id);
    const swapIdx = direction === "up" ? idx - 1 : idx + 1;
    if (idx < 0 || swapIdx < 0 || swapIdx >= scenarios.length) return;
    const batch = writeBatch(db);
