...
function TestCaseItem({ tcId, result, onStatus, onNoteBlur, }) {
    ...
    return (_jsxs("div", { className: "bg-white border border-gray-200 rounded-xl p-4 space-y-3", children: [_jsxs("div", { className: "flex items-start justify-between gap-2", children: [_jsx("p", { className: "text-sm font-medium text-gray-900", children: result.title }), _jsxs("div", { className: "flex items-center gap-1.5 shrink-0", children: [hasSteps && (_jsx("button", { onClick: () => setShowSteps((v) => !v), className: "text-[11px] text-indigo-600 hover:text-indigo-800 font-medium px-2 py-0.5 rounded-md hover:bg-indigo-50 transition-colors", children: showSteps ? "Hide steps" : "View steps" })), result.status !== "untested" && (_jsx("span", { className: cn("text-[11px] font-semibold rounded-full px-2.5 py-0.5 capitalize", TEST_STATUS_COLOR[result.status] ?? "bg-gray-100 text-gray-600"), children: result.status }))] })] }), showSteps && hasSteps && (_jsx("ol", { className: "space-y-1 pl-1", children: result.steps.map((step, i) => (_jsxs("li", { className: "flex gap-2 text-xs text-gray-600", children: [_jsxs("span", { className: "shrink-0 w-4 text-gray-400 font-medium text-right", children: [i + 1, "."] }), _jsx("span", { children: step })] }, i))) })), _jsx("div", { className: "grid grid-cols-4 gap-1.5", children: STATUS_BUTTONS.map(({ label, status, color }) => (_jsx("button", { onClick: () => onStatus(tcId, status), className: cn("py-2 text-xs font-semibold rounded-lg transition-all", result.status === status
