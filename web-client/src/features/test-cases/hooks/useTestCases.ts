...
async function incrementMonthlyUsage(uid: string, count: number) {
  const now = new Date();
...
export function useTestCases(
  ...
  async function createTestCase(title: string) {
    ...
    const result = await addDoc(collection(db, "test_cases"), {
      ...
      title,
      steps: [],
      status: "untested" as TestCaseStatus,
  ...
  async function updateTestCase(
    id: string,
    data: Partial<Pick<TestCase, "title" | "steps" | "status">>,
  ) {
  ...
  async function bulkCreateTestCases(
    items: { title: string; steps: string[]; status: TestCaseStatus }[],
  ) {
    if (!user || !projectId || !scenarioId || items.length === 0) return;
  ...
  async function moveTestCase(id: string, direction: "up" | "down") {
    const idx = testCases.findIndex((tc) => tc.id === id);
    const swapIdx = direction === "up" ? idx - 1 : idx + 1;
    if (idx < 0 || swapIdx < 0 || swapIdx >= testCases.length) return;
