...
export function useMembers(project) {
    ...
    async function inviteMember(projectId, email, role) {
        ...
        if (snap.empty)
            throw new Error('Pengguna tidak ditemukan. Minta mereka mendaftar terlebih dahulu.');
    ...
    async function changeMemberRole(projectId, uid, role) {
        await updateDoc(doc(db, 'projects', projectId), {
            [`members.${uid}`]: role,
        });
    }
